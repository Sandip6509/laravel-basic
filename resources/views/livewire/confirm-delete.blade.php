<div class="overflow-hidden mt-5 rounded-lg">
    <table class=" min-w-full border-collapse border border-gray-200 rounded-lg">
        <thead>
            <tr class="bg-gray-100 text-gray-700">
                <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> No
                </th>
                <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Name
                </th>
                <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Email
                </th>
        ...
        <tbody class="divide-y divide-gray-300 ">
            @foreach ($users as $user)
                <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 ">{{ $user->id }}
                    </td>
                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ $user->name }}
                    </td>
                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ $user->email }}
                    </td>
                    <td class=" p-5 ">
                        <div class="flex items-center gap-1">
                            <button wire:click="handleConfirmDelete({{ $user->id }})"
    ...
    <div wire:ignore.self class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 hidden" id="deleteModal">
        <div class="bg-white rounded-lg shadow-lg w-96">
            <div class="px-6 py-4 border-b flex justify-between items-center">
                <h5 class="text-lg font-semibold">Delete Confirmation</h5>
                <button class="text-gray-500 hover:text-gray-700" onclick="closeDeleteModal()">&times;</button>
            </div>
            <div class="p-6">
            ...
            <div class="px-6 py-4 border-t flex justify-end space-x-2">
                ...
                <button wire:click="delete" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Yes, Delete</button>
            </div>
        </div>
    </div>    
</div>
